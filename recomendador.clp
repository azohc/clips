...
(defrule recomiendaSocial
    ?ha <- (app (nombre ?na) (categoria SOCIAL))
    ?hbv <- (appBienValorada ?nbv)
    ?hmd <- (masDescargada ?nmd)
    ?hu <- (usuario (nombre ?nu) (edad ?eu) (usos_movil Social))
...
(defrule recomiendaParaNenes
    ?ha <- (app (nombre ?na) (content_rating Everyone))
    ?hbv <- (appBienValorada ?nbv)
    ?hu <- (usuario (nombre ?nu) (edad ?eu))
...
(defrule recomiendaParaAdolescentes
    ?ha <- (app (nombre ?na) (content_rating Teen))
    ?hbv <- (appBienValorada ?nbv)
    ?hmd <- (masDescargada ?nmd)
    ?hu <- (usuario (nombre ?nu) (edad ?eu))
...
(defrule recomiendaEducacion
    ?ha <- (app (nombre ?na) (categoria EDUCATION))
    ?hbv <- (appBienValorada ?nbv)
    ?hu <- (usuario (nombre ?nu) (edad ?eu))
...
(defrule recomiendaJovenGamers
    ?ha <- (app (nombre ?na) (categoria GAMES) (content_rating ?cr))
    ?hbv <- (appBienValorada ?nbv)
    ?hmd <- (masDescargada ?nmd)
    ?hu <- (usuario (nombre ?nu) (edad ?eu))
